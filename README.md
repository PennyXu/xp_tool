这是一个Python工具包，集成了RAG（检索增强生成）知识库构建、AI模型调用、云存储操作和数据处理等多种功能。特别适用于需要结合外部知识（如数据库Schema、业务文档）的智能对话和数据管理场景。
...
#### AsyncCallAiTqdm - 异步调用(实时进度条) 🆕
- **实施进度条**：在AsyncCallAi 类的基础上增加了实时进度条，适用于需要实时进度条、用户需感知运行状态的批量任务。
